...
面向城市公厕管养单位的巡查记录与整改闭环管理系统，覆盖 **公厕台账 → 保洁巡查 → 问题上报 → 整改跟踪 → 群众反映与热线转办** 五条业务主线。后端为 FastAPI + SQLAlchemy，前端为 React + Vite，前后端均按模块拆分，可单独开发、单独部署。
...
| 公厕台账 | `/restrooms`、`/restrooms/:id` | 台账增删改查、区域与状态筛选、公厕详情（档案 + 历史巡查 + 历史问题 + 群众反映）、关联数据删除保护 |
...
| 群众反映 | `/complaints`、`/complaints/:id` | 热线/12345/现场/网络多渠道受理登记、按内容智能判定分类、分派处理人、办结登记处理结果、回访反映人（未联系上可约再次回访）、办理轨迹时间线 |

其他页面不会互相混杂：台账、巡查、问题、诉求各自独立成页，详情页再做跨模块的关联展示。
...
│   │   ├── models               # ORM 模型：公厕、巡查、问题、整改流水、群众诉求与回访
│   │   ├── schemas              # Pydantic 出入参模型
│   │   ├── services             # 业务规则层：台账、巡查、问题整改、诉求办理、评分、统计
│   │   ├── seed.py              # 演示数据生成
...
| GET | `/complaints` | 诉求查询（status/source/category/district/visit_due/open_only/日期区间/关键字） |
| POST | `/complaints` | 受理登记，自动生成编号 `FS-YYYYMMDD-001`，分类留空时按内容自动判定 |
| POST | `/complaints/classify` | 按反映内容智能判定分类，返回命中关键词 |
| GET/PATCH/DELETE | `/complaints/{id}` | 详情（含办理轨迹与回访记录）/ 更新 / 删除 |
| GET | `/complaints/{id}/transitions` | 当前状态可执行的办理动作 |
| POST | `/complaints/{id}/transitions` | 推进办理状态（办结待回访时必填处理结果） |
| POST | `/complaints/{id}/visits` | 登记回访（满意办结 / 不满意退回 / 未联系上需约再次回访） |
...
- **删除保护**：删除公厕时若已存在巡查、问题或群众反映记录，接口返回 409 并提示数量，需要显式 `force=true` 才会级联删除；前端会二次确认。
- **诉求受理编号**：`FS-` + 登记日期 + 当日三位流水号。
- **诉求分类判定**：登记时不选分类则按反映内容的关键词命中数自动判定（保洁卫生/设施损坏/异味扰民/耗材缺失/安全隐患/服务态度/其他），也可随时调用 `/complaints/classify` 单独判定；前端在内容失焦时自动预填，人工选择优先。
- **诉求办理闭环**：`待受理 → 处理中 → 待回访 → 已办结 → 已关闭`；办结前必须登记处理结果。`待回访` 只能通过登记回访推进：满意则办结、不满意退回 `处理中` 重新办理、未联系上则保持 `待回访` 并约定下次回访时间（到点后进入「当前需回访」列表）。每次办理与回访都会写入办理流水，详情页以时间线呈现。

## 演示数据

`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录、13 条不同整改阶段的问题及其完整整改轨迹、8 件覆盖各办理阶段的群众反映（含回访与再次回访安排）。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。
