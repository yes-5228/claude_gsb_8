...
面向城市公厕管养单位的巡查记录与整改闭环管理系统，覆盖 **公厕台账 → 保洁巡查 → 问题上报 → 整改跟踪 → 诉求受理** 五条业务主线。后端为 FastAPI + SQLAlchemy，前端为 React + Vite，前后端均按模块拆分，可单独开发、单独部署。
...
| 公厕台账 | `/restrooms`、`/restrooms/:id` | 台账增删改查、区域与状态筛选、公厕详情（档案 + 历史巡查 + 历史问题 + 诉求记录）、关联数据删除保护 |
...
| 诉求受理 | `/complaints`、`/complaints/:id` | 群众反映与热线转办统一受理、按内容自动判定分类、分派/改派处理人、登记处理结果、回访反映人、未联系上约定再次回访、回访到时提醒 |
...
├── backend
│   ├── app
│   │   ├── api/v1/endpoints      # 路由层：restrooms / inspections / issues / complaints / stats / meta
│   │   ├── core                 # 配置、数据库、业务常量、领域异常
│   │   ├── models               # ORM 模型：公厕、巡查、问题、整改流水、诉求、办理流水、回访记录
│   │   ├── schemas              # Pydantic 出入参模型
│   │   ├── services             # 业务规则层：台账、巡查、问题整改、诉求受理、评分、统计
│   │   ├── seed.py              # 演示数据生成
...
│   │   ├── pages                # dashboard / restrooms / inspections / issues / complaints 五个模块
│   │   ├── utils                # 时间格式化、评分换算
│   │   └── styles/global.css
│   ├── nginx.conf
│   └── Dockerfile
...
| GET | `/complaints` | 诉求列表（source/category/status/district/keyword/follow_due/日期区间） |
| POST | `/complaints` | 受理登记，自动生成编号 `SQ-YYYYMMDD-001`，分类留空按内容自动判定 |
| POST | `/complaints/classify` | 按反映内容预判分类（登记表单实时提示用） |
| GET/PATCH/DELETE | `/complaints/{id}` | 详情（含办理流水与回访记录）/ 补正登记信息 / 删除 |
| POST | `/complaints/{id}/assign` | 分派处理人（待分派→处理中）；处理中再次调用为改派 |
| POST | `/complaints/{id}/finish` | 登记处理结果（处理中→待回访） |
| POST | `/complaints/{id}/follow-ups` | 登记回访：联系上则办结；未联系上须约定下次回访时间 |
| POST | `/complaints/{id}/close` | 作废关闭（仅待分派/处理中可关闭） |
...
- **诉求受理闭环**：`待分派 → 处理中 → 待回访 → 已办结`；`待分派/处理中` 可作废关闭，`处理中` 可改派处理人。受理登记、分派、登记结果、每次回访都会写入办理流水，详情页以时间线呈现。
- **诉求自动分类**：受理登记时分类留空，服务端按反映内容命中关键词自动判定（异味扰民/设施损坏/耗材缺失/服务态度/保洁卫生/其他），可手动改判；补正内容且未指定分类时会重新判定。
- **回访与再次回访**：处理完成后登记处理结果进入回访环节。回访「已联系上」记录满意度并办结；「未联系上」必须约定下次回访时间，约定时间已过仍未回访的诉求在列表以「待再次回访」标签提醒，也可用 `follow_due=true` 筛选。
- **删除保护**：删除公厕时若已存在巡查或问题记录，接口返回 409 并提示数量，需要显式 `force=true` 才会级联删除；前端会二次确认。

## 演示数据

`SEED_ON_STARTUP=true`（默认）且数据库为空时，会自动写入：10 座公厕（4 个区域、三类等级、含维修/停用状态）、近 14 天约 90 条巡查记录、13 条不同整改阶段的问题及其完整整改轨迹、10 条覆盖各办理阶段的群众诉求（含回访记录与待再次回访提醒）。数据由固定随机种子生成，结果可复现；如需重置，删除 `backend/data/app.db`（或 `docker compose down -v`）后重启即可。

## 测试与验证

```bash
cd backend && pytest -q          # 接口测试（覆盖台账 CRUD、删除保护、评分、流程流转、诉求受理与回访、统计）
cd frontend && npm run build     # 生产构建
```

本项目完成时已实际运行验证：

- 后端 `pytest`：9 个用例全部通过；`/health`、台账/巡查/问题/诉求/统计/字典接口均返回预期数据。
