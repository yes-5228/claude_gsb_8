...
from app.core.constants import OPEN_COMPLAINT_STATUSES, OPEN_ISSUE_STATUSES
from app.core.exceptions import ConflictError, DomainError, NotFoundError
from app.models import Complaint, Inspection, Issue, Restroom
...
def delete_restroom(db: Session, restroom_id: int, *, force: bool = False) -> None:
    ...
    complaint_count = db.scalar(
        select(func.count()).select_from(Complaint).where(Complaint.restroom_id == restroom_id)
    ) or 0
    if (inspection_count or issue_count or complaint_count) and not force:
        raise ConflictError(
            f"该公厕已有 {inspection_count} 条巡查记录、{issue_count} 条问题记录、"
            f"{complaint_count} 条诉求记录，确需删除请使用 force=true"
        )
    db.delete(restroom)
    db.commit()
...
def get_restroom_detail(db: Session, restroom_id: int) -> RestroomDetail:
    ...
    open_complaint_count = db.scalar(
        select(func.count())
        .select_from(Complaint)
        .where(Complaint.restroom_id == restroom_id, Complaint.status.in_(OPEN_COMPLAINT_STATUSES))
    ) or 0
    total_complaint_count = db.scalar(
        select(func.count()).select_from(Complaint).where(Complaint.restroom_id == restroom_id)
    ) or 0
